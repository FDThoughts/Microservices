namespace eShop.EventsModule.EventBusRabbitMQ.V1
{
    ...
    using eShop.EventsModule.EventBus.V1.Extensions;
    ...
    public class EventBusRabbitMQ : IEventBus, IDisposable
    {
        ...
        public EventBusRabbitMQ(IRabbitMQPersistentConnection 
            persistentConnection, ILogger<EventBusRabbitMQ> logger,
            ...
            where TH : IIntegrationEventHandler<T>
        {
            ...
            _logger.LogInformation(
                "Unsubscribing from event {EventName}", 
                eventName);
        ...
        private void StartBasicConsume()
        {
            ...
            {
                _logger.LogError(
                    "StartBasicConsume can't call on _consumerChannel == null");
            }
        }
        ...
        {
            _logger.LogTrace(
                "Processing RabbitMQ event: {EventName}", eventName);
            ...
            if (_subsManager.HasSubscriptionsForEvent(eventName))
            {
                using (var scope = _autofac.BeginLifetimeScope(
                    AUTOFAC_SCOPE_NAME))
                {
                    var subscriptions = _subsManager.GetHandlersForEvent(
                        eventName);
                    foreach (var subscription in subscriptions)
                    {
                        ...
                        else
                        {
                            var handler = scope.ResolveOptional(
                                    subscription.HandlerType);
                            if (handler == null) continue;
                            var eventType = _subsManager.GetEventTypeByName(
                                eventName);
            ...
            {
                _logger.LogWarning(
                    "No subscription for RabbitMQ event: {EventName}", 
                    eventName);
            }
        }
    }
